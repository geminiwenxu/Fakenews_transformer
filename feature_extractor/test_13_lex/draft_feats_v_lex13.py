...
class Article():
    def __init__(self, raw_text, dict_positive, dict_negative, dict_style, dict_anto, dict_topo,
        ...
        self.dict_SCAN = dict_SCAN
        self.dict_SUSP = dict_SUSP
        self.dict_EXPO = dict_EXPO
        self.dict_DIST = dict_DIST
        self.dict_CONS = dict_CONS
        self.dict_ASEM = dict_ASEM
        self.dict_ELIT = dict_ELIT
        self.dict_APOC = dict_APOC
        self.dict_PROT = dict_PROT
        self.dict_NAT = dict_NAT
        self.dict_ISLA = dict_ISLA
        self.dict_GEND = dict_GEND
        self.dict_ESO = dict_ESO
        ...
        self.SCAN, self.SUSP, self.EXPO, self.DIST, self.ASEM, self.CONS, self.ELIT, self.APOC, self.PROT, self.NAT, \
        self.ISLA, self.GEND, self.ESO = self.get_scores()
    ...
    def get_scores(self):
        ...
        for t in self.doc:
            if t.is_stop is False and t.is_space is False:
                if (t.text.lower() in self.dict_SCAN): self.count_SCAN += 1
                if (t.text.lower() in self.dict_SUSP): self.count_SUSP += 1
                if (t.text.lower() in self.dict_EXPO): self.count_EXPO += 1
                if (t.text.lower() in self.dict_DIST): self.count_DIST += 1
                if (t.text.lower() in self.dict_CONS): self.count_CONS += 1
                if (t.text.lower() in self.dict_ASEM): self.count_ASEM += 1
                if (t.text.lower() in self.dict_ELIT): self.count_ELIT += 1
                if (t.text.lower() in self.dict_APOC): self.count_APOC += 1
                if (t.text.lower() in self.dict_PROT): self.count_PROT += 1
                if (t.text.lower() in self.dict_NAT): self.count_NAT += 1
                if (t.text.lower() in self.dict_ISLA): self.count_ISLA += 1
                if (t.text.lower() in self.dict_GEND): self.count_GEND += 1
                if (t.text.lower() in self.dict_ESO): self.count_ESO += 1

        return self.count_SCAN / len(self.doc), \
               self.count_SUSP / len(self.doc), \
               self.count_EXPO / len(self.doc), \
               self.count_DIST / len(self.doc), \
               self.count_ASEM / len(self.doc), \
               self.count_CONS / len(self.doc), \
               self.count_ELIT / len(self.doc), \
               self.count_APOC / len(self.doc), \
               self.count_PROT / len(self.doc), \
               self.count_NAT / len(self.doc), \
               self.count_ISLA / len(self.doc), \
               self.count_GEND / len(self.doc), \
               self.count_ESO / len(self.doc)
