...
def attention_fusion(bert_embedding, feature_embedding):
    ...
    for layer in range(attention_layer):
        for _ in range(head):
            feature_key = torch.randn(64, 32)
            text_query = torch.randn(768, 32)
            feature_value = torch.randn(64, 32)
            ...
            text_key = torch.randn(768, 32)
            feature_query = torch.randn(64, 32)
            text_value = torch.randn(768, 32)
